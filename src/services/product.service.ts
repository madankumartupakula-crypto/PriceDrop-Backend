...
export async function trackProductForUser(userId: string, input: TrackProductInput): Promise<IProduct> {
  ...
  if (user && product.notificationEnabled && product.targetPrice !== null && product.targetPrice !== undefined) {
    if (product.currentPrice <= product.targetPrice && product.notificationSettings.targetPriceAlert) {
